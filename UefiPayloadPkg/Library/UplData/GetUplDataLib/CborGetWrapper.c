...
{
  RETURN_ON_ERROR_CBOR_DECODE (cbor_parser_init (Buffer, Size, 0, &Parser, &RootValue));
  ...
  )
{
  ...
  RETURN_ON_ERROR_CBOR_DECODE (cbor_value_get_simple_type (&Element, Result));
  ...
  )
{
  ...
  RETURN_ON_ERROR_CBOR_DECODE (cbor_value_map_find_value (&RootValue, String, &Array));
