  ...
  )
{
  ...
  if (!EFI_ERROR (Status)) {
    NewSmbiosTable = BuildGuidHob (&gUniversalPayloadSmbiosTableGuid, sizeof (SmbiosTable));
  ...
  if (!EFI_ERROR (Status)) {
    NewAcpiTableHob = BuildGuidHob (&gUniversalPayloadAcpiTableGuid, sizeof (AcpiTableHob));
