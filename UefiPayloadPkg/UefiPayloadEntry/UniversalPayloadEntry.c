  ...
  )
{
 ...
 // GetCbor (GET_GUID_HOB_DATA (GuidHob), GET_GUID_HOB_DATA_SIZE(GuidHob));
  ...
  CBorRootmap = GetFromCborRootMapInit(GET_GUID_HOB_DATA (GuidHob), GET_GUID_HOB_DATA_SIZE(GuidHob));

  element = GetFromCborValueInMap("SerialPortBaudRate",CBorRootmap);
  GetFromCborUint64(element,&result);
  Serial->BaudRate       = (UINT32)result;

  element = GetFromCborValueInMap("SerialPortRegisterBase",CBorRootmap);
  GetFromCborUint64(element,&result);
  Serial->RegisterBase       = (UINT32)result;

  element = GetFromCborValueInMap("SerialPortRegisterStride",CBorRootmap);
  GetFromCborUint64(element,&result);
  Serial->RegisterStride       = (UINT32)result;

  element = GetFromCborValueInMap("SerialPortUseMmio",CBorRootmap);
  GetFromCborBoolean(element,&boolresult);
